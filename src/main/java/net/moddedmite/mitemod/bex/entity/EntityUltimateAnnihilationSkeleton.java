...
public class EntityUltimateAnnihilationSkeleton extends EntitySkeleton implements IBossDisplayData {
...
//    private Enchantment[] nonLevelsBookList = new Enchantment[]{Enchantments.enchantmentFixed, Enchantments.enchantmentChain, Enchantments.EMERGENCY, Enchantments.EnchantmentForge};
    ...
    private final ItemStack weapon = Utils.safeMake(() -> {
        ItemStack itemStack = new ItemStack(BEXItems.ENDING);
        itemStack.addEnchantment(Enchantment.knockback, 10);
        itemStack.addEnchantment(MITEITEEnchantmentRegistryInit.EXTEND, 5);
        itemStack.addEnchantment(Enchantment.fireAspect,5);
        return itemStack;
        }, null);

    protected void addRandomEquipment() {
        this.setCurrentItemOrArmor(0, new ItemStack(BEXItems.ENDING, 1).randomizeForMob(this, true));
        this.setCurrentItemOrArmor(1, new ItemStack(BEXItems.unassailableHelmet, 1).randomizeForMob(this, true));
        this.setCurrentItemOrArmor(2, new ItemStack(BEXItems.unassailableChestPlate, 1).randomizeForMob(this, true));
        this.setCurrentItemOrArmor(3, new ItemStack(BEXItems.unassailableLeggings, 1).randomizeForMob(this, true));
        this.setCurrentItemOrArmor(4, new ItemStack(BEXItems.unassailableBoots, 1).randomizeForMob(this, true));
...
//    }
    ...
    public void onLivingUpdate() {
        ...
        if (particleCount > 0) {
            ...
            this.spawnParticle(EnumParticle.largesmoke, 10);
            this.spawnParticle(EnumParticle.largesmoke, 10);
            this.spawnParticle(EnumParticle.largesmoke, 10);
            this.spawnParticle(EnumParticle.largesmoke, 10);
            this.spawnParticle(EnumParticle.largesmoke, 10);
            ...
            if (ticksExisted % 20 == 0) {
                ...
                if(target instanceof EntityPlayer) {
                    if (hasClonePlayer == false) {
                        if (target.getMaxHealth() > this.getMaxHealth()) {
                            this.setEntityAttribute(SharedMonsterAttributes.maxHealth, target.getMaxHealth());
                    ...
                    while (var7.hasNext()) {
                        ...
                        this.addPotionEffect(new PotionEffect(var8.getPotionID(), var8.getDuration()));
                    }
                ...
                if(thunderTick % 20 == 0) {
                    if(target != null && target instanceof EntityPlayer) {
                        if(((IEXPlayer) target).getIsAttackByBossCounter() <= 0) {
        ...
        if (this.targetedEntity != null && this.targetedEntity.getDistanceSqToEntity(this) < var9 * var9) {
            ...
            if (super.canSeeEntity(this.targetedEntity)) {
                ...
                if (this.attackCounter == 15) {
                    ...
                    //Triple shot
                    for(int i = 3; i > 0; --i) {
                        var var17 = new EntityLargeFireballNB(super.worldObj, this, target_center, 4.0F + (float)i * 2.5F);
...
//                    this.setDead();
    ...
    protected void applyEntityAttributes() {
        ...
        this.setEntityAttribute(SharedMonsterAttributes.maxHealth, Integer.MAX_VALUE);
        this.setEntityAttribute(SharedMonsterAttributes.attackDamage, 0.0F);
        this.setEntityAttribute(SharedMonsterAttributes.movementSpeed, 0.5D);
        this.setEntityAttribute(SharedMonsterAttributes.followRange, 128.0D);
    }

    @Override
    public EntityDamageResult attackEntityFrom(Damage damage) {
        ITEDamage iteDamage = (ITEDamage) damage;
        ...
        if (can_evade && this.num_evasions > 0) {
            ...
            if (damage.isIndirect() || !damage.isMelee() || damage.isExplosion() || damage.isArrowDamage() || !(iteDamage.getResponsibleEntityP() instanceof EntityPlayer)) {
                damage.setAmount(0f);
            } else if (iteDamage.getResponsibleEntityP() instanceof EntityPlayer) {
                attackedByPlayer = true;
                if (Math.abs(this.getFootPosY() - ((EntityPlayer) iteDamage.getResponsibleEntityP()).getFootPosY()) >= 3.0D) {
                    damage.setAmount(0f);
                    return null;
                }
    ...
    protected void dropFewItems(boolean recently_hit_by_player, DamageSource damage_source) {
        if (recently_hit_by_player){
            this.dropItem(BEXItems.voucherUltimateAnnihilation);
