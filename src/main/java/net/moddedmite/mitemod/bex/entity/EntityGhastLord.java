...
public class EntityGhastLord extends EntityFlying implements IMob {
    ...
    protected void applyEntityAttributes() {
        ...
        super.getEntityAttribute(SharedMonsterAttributes.maxHealth).setAttribute(200.0D);
    }
        ...
        if (this.targetedEntity != null && this.targetedEntity.getDistanceSqToEntity(this) < var9 * var9) {
            ...
            if (super.canSeeEntity(this.targetedEntity)) {
                ...
                if (this.attackCounter == 15) {
                    ...
                    //Triple shot
                    for (int i = 3; i > 0; --i) {
                        var fireball = new EntityLargeFireballNB(super.worldObj, this, target_center, 4.0F + (float)i * 2.5F);
                        if (this.worldObj.isOverworld()) {
                            fireball.field_92057_e = Math.round((float)this.explosionStrength * 3.0F);
                        } else {
    ...
    protected void dropFewItems(boolean recently_hit_by_player, DamageSource damage_source) {
        if (recently_hit_by_player) {
            this.dropItem(BEXItems.voucherGhast);
